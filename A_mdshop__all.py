...
def run_pre_match_script():
    try:
        ...
        display_prematch_data(df)
        update_status("Pre-match data saved successfully!", 100)

    except WebDriverException as e:
        update_status(f"Error: {e}")

# Display the pre-match data in a table
...
# Run Pre-Match button
run_prematch_button = tk.Button(root, text="Run Pre-Match Script", command=lambda: threading.Thread(target=run_pre_match_script).start())
run_prematch_button.pack(pady=5)
